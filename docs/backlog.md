...
### Линтер: revive с правилом exported
`infra` `size:XS`

**Что делать.** Добавить `revive` в `.golangci.yml` и включить правило
`exported`, проверяющее наличие комментариев у экспортируемых сущностей.

**Зачем.** В #94 нашёлся тип `ListParams`, документация которого уезжала к
соседнему `const`-блоку: `go doc` показывал её пустой. Машиной это не
поймалось, потому что в конфиге стоит `default: standard`, а проверка
комментариев живёт в `revive`, которого там нет.

**Трогает.** `.golangci.yml`, возможно точечные правки комментариев.

**Зависимости.** Нет.

**Критерии приёмки.**

- `make lint` падает, если у экспортируемой сущности нет комментария.
- Существующий код проходит проверку без исключений в конфиге либо каждое
  исключение объяснено комментарием рядом.