...
- `STRICT_OWNER_CHECK` по умолчанию `true`. Выключайте только для локальных
  демо: при `false` отсутствие заголовка включает anon-режим без ACL на чтение.
...
**Периметр доверия.** `owner_id` и `media_id` в payload **не валидируются**
относительно издателя события: consumer доверяет JSON как есть. После того
как привязка даёт право на чтение (`GetMedia` / download / presign), запись
в топик равносильна выдаче ACL. Поэтому `KAFKA_TOPIC` должен быть доступен
только доверенным внутренним продюсерам (сеть, ACL брокера, SASL). Это
осознанный контракт v1, а не дыра «на потом забыли»; криптоподпись событий
или проверка источника - отдельное усиление.
