...
# Держать в синхроне с .github/workflows/ci.yml (Ensure generated code).
...
proto: ## Сгенерировать Go stubs из proto (идемпотентно)
	...
	@which protoc-gen-go > /dev/null || (echo "ERROR: protoc-gen-go не установлен. Запусти: go install google.golang.org/protobuf/cmd/protoc-gen-go@$(PROTOC_GEN_GO_VERSION)" && exit 1)
	@which protoc-gen-go-grpc > /dev/null || (echo "ERROR: protoc-gen-go-grpc не установлен. Запусти: go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@$(PROTOC_GEN_GO_GRPC_VERSION)" && exit 1)
