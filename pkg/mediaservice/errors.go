...
// с другими параметрами загрузки: иным MIMEType, другими флагами обработки,
// другим TTL или другим заявленным ExpectedSize.
//
// Содержимое файла при этом не сверяется. Повтор с тем же ключом, теми же
// параметрами и тем же заявленным размером считается тем же запросом:
// вернётся существующий объект без ошибки, даже если байты другие. Байты
// в этом случае не читаются вовсе - ответ отдаётся до приёма тела.
//
// Полагаться на ключ идемпотентности как на защиту от подмены содержимого
// поэтому нельзя. Если это важно, вызывающий обязан сам сделать ключ
// производным от содержимого - например, взять хеш файла.
