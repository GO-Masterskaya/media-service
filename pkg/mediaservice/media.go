...
//
// Доступ имеет владелец (media.owner_id) или attacher с записью в
// media_attachments; иначе ErrAccessDenied.
// В отличие от gRPC-контракта, где проверку caller можно отключить
...
// объекта, доступ (владелец media.owner_id или привязка в media_attachments),
// доступность варианта - выполняются до возврата, поэтому поток без ошибки
// означает, что доступ подтверждён, а сбой при чтении относится к сети или
// хранилищу.
//
