...
func (s *MediaServer) Upload(stream mediav1.MediaService_UploadServer) error {
	...
	// 2. Функция получения чанков из стрима
	chunkRecv := func() ([]byte, error) {
		req, err := stream.Recv()
		if err != nil {
			return nil, err // io.EOF или сетевая ошибка
		}
		if req.GetInit() != nil {
	...
	if err != nil {
		...
		if errors.Is(err, upload.ErrDiskFull) {
			return status.Error(codes.ResourceExhausted, "insufficient server disk space")
		}
