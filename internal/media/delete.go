...
//   - true  — довести очистку до конца самому. Используется DeleteByOwner —
//     но на практике ClaimAlreadyDeleting туда почти не долетает:
//     ListDeletableByOwner уже фильтрует status <> 'deleting' на входе, так
//     что до этой ветки доходит только настоящая гонка (запись сменила
//     статус между ListDeletableByOwner и этим вызовом), а не "резюме
//     собственной зависшей записи с прошлой попытки" — тот сценарий сюда
//     физически не долетает, он отфильтрован раньше (см. ревью PR #13/#17).
...
// deleteByID-команду (resumeStuck=true — это явный единичный вызов,
// а не периодический фоновый обход). batchSize<=0 ->
